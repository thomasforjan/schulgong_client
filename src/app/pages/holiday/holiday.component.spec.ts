...
describe('HolidayComponent', () => {
  ...
  beforeEach(async () => {
    ...
    const holidayBackendService = jasmine.createSpyObj(
      'HolidayBackendService',
      ['getHolidayResponse']
    );
    // Make the spy return a synchronous Observable with the test data
    getHolidaySpy = holidayBackendService.getHolidayResponse.and.returnValue(
      of(testHoliday)
    );
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        { provide: HolidayBackendService, useValue: holidayBackendService },
      ],
    }).compileComponents();
  ...
  describe('holiday overview test', () => {
    ...
    it('check card-header exist', async () => {
      const card = await loader.getHarness(
        MatCardHarness.with({ title: testHoliday[0].name })
      );
    ...
    it('check card-title', () => {
      ...
      ourDomCardsUnderTest.forEach((card) => {
        const cardTitle =
          card.getElementsByTagName('mat-card-title')[0].textContent;
    ...
    it('check card-content', async () => {
      ...
      ourDomCardsUnderTest.forEach((card) => {
        const cardContent =
          card.getElementsByTagName('mat-card-content')[0].textContent;
        if (cardContent != null) {
          expect(cardContent.trim()).toBe(
            testHoliday[i].startDate.toLocaleDateString('de-DE', {
              day: '2-digit',
              month: '2-digit',
              year: 'numeric',
            }) +
    ...
    it('check onclick delete button', fakeAsync(async () => {
      ...
      testHoliday.forEach((holiday) => {
        gridCardssComponent.onDelete(testHoliday[0].id, event);
      });
