describe('Klingelzeit Test', () => {
  it('Visit the ringtime page', () => {
    ...
    cy.get(`[aria-label="addRingtime"]`).children(`[aria-label="ariaLabelText"]`).click()
    ...
    cy.get(`[aria-label="ringtimeSaveButton"]`).children(`[aria-label="ariaLabelText"]`)
    cy.get(`[aria-label="ringtimeCancelButton"]`).children(`[aria-label="ariaLabelText"]`)

    //  Get an input, type into it
    cy.get(`[aria-label="ringtimeName"]`).type('Beginn 2.Stunde')

    //  Verify that the value has been updated
    cy.get(`[aria-label="ringtimeName"]`).find('input').should('have.value', 'Beginn 2.Stunde')

    //  Get an input, type into it
    cy.get(`[aria-label="ringtimeStartdate"]`).type('2022-09-01')
    //  Verify that the value has been updated
    cy.get(`[aria-label="ringtimeStartdate"]`).find('input').should('have.value', '2022-09-01')

    //  Get an input, type into it
    cy.get(`[aria-label="ringtimeEnddate"]`).type('2023-07-30')
    //  Verify that the value has been updated
    cy.get(`[aria-label="ringtimeEnddate"]`).find('input').should('have.value', '2023-07-30')
  });
});
